...
PS1user="$( test `whoami` == root && echo '\[\e[101m\]' )\h\[\e[0m\]"
case $TERM in
    xterm*)
    #export PS1="$PS1error[\D{%H:%M}][$PS1user][\W]${Red}\$(__git_ps1)${Color_Off}> \[\e]2;[\w]\a\]"
    export PS1="$PS1error[\D{%H:%M}][$PS1user][\W]\$(__git_ps1)> \[\e]2;[\w]\a\]"
    ;;
    *)
    export PS1="$PS1error[\D{%H:%M}][$PS1user][\W]\$(__git_ps1)>\$"
    ;;
esac
